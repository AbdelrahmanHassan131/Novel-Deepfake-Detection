"""
Concatenation Fusion Trainer (Fusion_128).
...
class ConcatenationFusionTrainer(BaseModel):
    """
    Trainer for Concatenation Fusion model.
    Combines pre-trained RGB (Wang2020) and Wavelet (Wolter2022) models using simple concatenation.
    """
    ...
    def __init__(self, opt):
        super(ConcatenationFusionTrainer, self).__init__(opt)
        ...
        if self.isTrain and not opt.continue_train:
            self.model = ConcatenationFusionClassifier(
        ...
        print(f"Concatenation Fusion model created")
